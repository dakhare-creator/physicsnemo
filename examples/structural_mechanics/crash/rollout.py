...
class TransolverAutoregressiveRolloutTraining_features(Transolver):
    """
    Transolver model with autoregressive rollout training.
    ...
    def forward(self, sample: SimSample, data_stats: dict) -> torch.Tensor:
        ...
        for t in range(self.rollout_steps):
            ...
            # Model input
            fx_t = torch.cat([y_t1_diff, vel_norm], dim=-1)  # [N, 3+F+1]

            embedding = torch.cat([features, collision_emb, damage_emb, diff], dim=-1)
